...
var $author$project$Main$wordsUrl = '../static/thousand_words_things_explainer.txt';
var $author$project$Main$fetchWords = $elm$http$Http$get(
	{
		expect: $elm$http$Http$expectString($author$project$Main$GotWords),
		url: $author$project$Main$wordsUrl
	});
...
var $author$project$Main$GotDefinitions = function (a) {
	return {$: 'GotDefinitions', a: a};
};
...
var $author$project$Main$meaningDecoder = A2(
	$elm$json$Json$Decode$field,
	'definitions',
	$elm$json$Json$Decode$list(
		A2($elm$json$Json$Decode$field, 'definition', $elm$json$Json$Decode$string)));
var $author$project$Main$entryDecoder = A2(
...
var $author$project$Main$fetchDefinition = function (word) {
	return $elm$http$Http$get(
		{
			expect: A2($elm$http$Http$expectJson, $author$project$Main$GotDefinitions, $author$project$Main$definitionsDecoder),
...
var $author$project$Main$parseWords = function (content) {
	return A2(
		$elm$core$List$filter,
		function (w) {
			return $elm$core$String$length(w) > 0;
		},
		...
		switch (msg.$) {
			case 'GotWords':
				...
				if (result.$ === 'Ok') {
					...
					var ws = $author$project$Main$parseWords(content);
					var maxIndex = $elm$core$List$length(ws) - 1;
					return (maxIndex < 0) ? _Utils_Tuple2(
						_Utils_update(
							model,
							{
								words: $author$project$Main$Success(_List_Nil)
							}),
						$elm$core$Platform$Cmd$none) : _Utils_Tuple2(
						_Utils_update(
							model,
							{
								words: $author$project$Main$Success(ws)
							}),
						A2(
							$elm$random$Random$generate,
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								words: $author$project$Main$Failure(e)
							}),
					...
					if (w.$ === 'Nothing') {
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									defs: $author$project$Main$Loading,
									pickedWord: $elm$core$Maybe$Just(word)
								}),
							$author$project$Main$fetchDefinition(word));
					}
				} else {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				}
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								defs: $author$project$Main$Success(ds)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var e = result.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								defs: $author$project$Main$Failure(e)
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'AnswerChanged':
			...
			if ((!_v0.b) && (_v0.c.$ === 'Just')) {
				...
				return A2(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('You lost! The word was: ' + word)
								])),
							$author$project$Main$maybeRefreshButton(
							$elm$core$Maybe$Just($author$project$Main$Refresh))
						]));
				...
				return A2(
					...
					_List_fromArray(
						[
							$elm$html$Html$text(
							'Wrong answer, try again! You still have ' + ($elm$core$String$fromInt(model.remainingTries) + ' tries.'))
						]));
			}
		}
	} else {
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Your answer:')
							])),
						A2(
						$elm$html$Html$input,
						_List_fromArray(
